...
export default class App extends React.PureComponent<undefined, AppState> {
  ...
  constructor(props: any) {
    ...
    for (let i = 0; i < sampleCards; i++) {
      ...
      cards[id] = {id: id, x: x, y: y, z: i, onBoard: true,
        image: imageCard ? sampleImages[Math.floor(Math.random() * sampleImages.length)] : null,
        text: imageCard ? null : sampleTexts[Math.floor(Math.random() * sampleTexts.length)]};
    }
    this.state = {cards: cards, highestBoardZ: sampleCards - 1, mouseX: 0, mouseY: 0};
  }
  ...
  liftBoardCardZ = (card: Card) => {
    ...
    const newState = Object.assign({}, this.state, {highestBoardZ: newHighestBoardZ, cards: newCards});
    this.setState(newState);
  }

  mouseDownArchiveCard = (card: Card) => {
    const newHighestBoardZ = 500000000000;
    const newCard = Object.assign({}, card, {z: newHighestBoardZ, isBeingDraggedFromArchive: true});
    const newCards = Object.assign({}, this.state.cards, {[card.id]: newCard});
    const newState = Object.assign({}, this.state, {highestBoardZ: newHighestBoardZ, cards: newCards});
    this.setState(newState);
  }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Archive cards={this.state.cards} mouseDownArchiveCard={this.mouseDownArchiveCard} />
        {optionalDiv}
      </div>
    );
  }
  ...
  onMouseMove = (e: React.SyntheticEvent) => {
    const newState = Object.assign({}, this.state, {mouseX: e.clientX, mouseY: e.clientY});
    this.setState(newState);
  }
    ...
    Object.keys(this.state.cards).map((id: string) => {
      ...
      if (card.isBeingDraggedFromArchive) {
        const newCard = Object.assign({}, card, {
          x: this.state.mouseX, y: this.state.mouseY, z: 5000000000, onBoard: true, isBeingDraggedFromArchives: false
        });
        newCards[card.id] = newCard;
      }
    });
    const newState = Object.assign({}, this.state, {cards: newCards, mouseX: e.clientX, mouseY: e.clientY});
    this.setState(newState);
  }
}
